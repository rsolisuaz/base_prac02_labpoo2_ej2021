...
public class ProcesadorDatosTest {	
    ...
    void testDatosMuestra() {
        ...
        assertEquals("$34.25", lineasResultado.get(0));
        assertEquals("$46.20", lineasResultado.get(1));
        assertEquals("$781.23", lineasResultado.get(2));
        assertEquals("$2,107.71", lineasResultado.get(3));
        assertEquals("$17,015.00", lineasResultado.get(4));
    ...
    void testDatosOtros() {
        ...
        List<String> lineasResultado = 
          proc.procesaListaNumeros(lineasEntrada,
            codigoIdioma, codigoPais);
        String[] res = {"$7.00","$21.00","$663.11","$881.10","$1,071.00","$1,945.00","$2,020.00","$92,231.36"};
    ...
    void testDatosSoloNums() {
        ...
        List<String> lineasResultado = 
          proc.procesaListaNumeros(lineasEntrada,
            codigoIdioma, codigoPais);
        String[] res = {"$10.20","$25.00","$114.00","$223.22","$532.00","$871.00","$1,976.00","$2,001.00","$5,155.12","$8,973.20"};
        assertEquals(res.length, lineasResultado.size());
        for (int i=0; i<res.length; i++) {
            assertEquals(res[i],lineasResultado.get(i));
        }
